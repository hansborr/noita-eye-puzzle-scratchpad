...
**The ingest front door (from brief 03)** — `parse_sequence(text, layer)` / `load_sequence(input, layer)` over `enum Input<'a> { Str(&'a str), Path(&'a Path) }` (no `Stdin` variant; the CLI reads stdin) cover the eye *digit* layers. Puzzle `one` (`{0..4}` digits) rides `parse_sequence`; puzzle `two` (`{A..L}`) rides `solve --alphabet ABCDEFGHIJKL --file research/data/practice-puzzles/two` → `Alphabet::from_chars` (`src/glyph.rs:165`) → `Sequence::parse` (`src/glyph.rs:219-231`); puzzle `six` (`{1..6}` digits **with spaces**) rides the general letter/transparent-symbol ingest path (brief 03/T3) where its spaces are configured **transparent symbols** (recorded by position, not `InvalidToken`), feeding only the cipher-digit stream into the codec. `Glyph` is `pub struct Glyph(pub u16)` (`src/glyph.rs:140`). **At test time use the checked-in corpus files (`research/data/practice-puzzles/`), never `/tmp`.**
...
9. **Corpus codec/grouping samples (`one`/`two`/`six`) + auto-logging.** All three read from the **checked-in corpus files** under `research/data/practice-puzzles/` (includable like other `research/data/...` assets), never `/tmp`.
   ...
   - **`six`** (`research/data/practice-puzzles/six`; English HYPOTHESIZED, NO cleartext): the **clearest base-N-grouping case** — `FixedGrouping { group_len: 2, base: 6 }` widens 6 digits to `6²=36 ≥ 26` so the mapping can host the language; its **preserved spaces exercise transparent-symbol passthrough** (brief 04/T2 — spaces recorded by position at ingest, passed through the codec unchanged, reinserted into `rendered_text`, skipped by the bigram scorer). `solve`+codec runs end-to-end and any surviving candidate is logged as a labelled HYPOTHESIS. Provenance noted (external sample, hypothesized recoverable English).
...
- **CLI smoke (manual, not a test):** `cargo run --locked -- solve --alphabet 01234 --codec-search --file research/data/practice-puzzles/one` runs end-to-end and prints a ranked, labelled-HYPOTHESIS result; `cargo run --locked -- solve --alphabet ABCDEFGHIJKL --codec-search --file research/data/practice-puzzles/two` likewise; `cargo run --locked -- solve --alphabet 123456 --codec-search --file research/data/practice-puzzles/six` exercises the base-6 grouping + transparent-space passthrough. **At test time use the checked-in corpus files, not `/tmp`.**
...
- **Lossy codecs must be honest.** A trailing partial group, or an accept-`0..=82` filter that drops raw `83..=124` values, makes a codec non-invertible: mark `is_invertible() == false`, set `codec_round_trip_ok = false`, and never claim a byte-for-byte round-trip it cannot provide. The raw-`0..=124`-vs-accepted-`0..=82` distinction is the same transcription-risk boundary brief 03/R3 names — never conflate the two alphabets.
...
- **Brief-02 / brief-04 / brief-07B coupling.** `AnyCipher` comes from 02; the `codec: AnyCodec` field and the pipeline stage live in brief 04's `Candidate`/`solve` (spec R2); the repo-wide role-directory move (relocating `codec.rs` under `src/attack/`) is brief 07B. If 02's `AnyCipher` or 04's `Candidate`/`solve` shapes differ from these sketches, follow those briefs and update cross-references here.
...
- **A raw `0..=124` trigram ingest layer.** This brief consumes accepted reading-layer symbols and the accept-`0..=82` policy; a general raw-trigram ingest is a separate, out-of-scope layer (brief 03/R3 names the same boundary).
