...
nulls/ ciphers/ attack/ experiments/ report/` (`00-OVERVIEW.md` §"Target module
layout"), so the file tree reflects the architecture instead of a single
...
(`00-OVERVIEW.md` §"Target module layout"). Use Rust's `path` attribute so the
**public paths stay flat** (`crate::analysis::…` keeps working) while the *files*
move into role dirs:
...
Grouping per `00-OVERVIEW.md` §"Target module layout". Each module appears in
exactly one directory, assigned by primary role; an experiment-vs-analysis-ambiguous
module is placed by primary role and may be re-homed when brief 08 wraps it as
`impl Experiment`.
...
- `analysis/` ← `analysis`, `isomorph`, `grouping`, `chaining`, `chaining_graph`,
  `perfect_isomorphism`, `honeycomb`, `orders`
...
- `experiments/` ← `periodicity`, `conditional_structure`, `modular_diff`,
  `orientation_homogeneity`, `transitivity`, `pyry_conditions`, `controls` (the
  structural-battery experiment drivers, each `impl Experiment`)
...
(`00-OVERVIEW.md` §"Read this overview first." — "names are proposals; verify
against code"). Where a module's role is ambiguous (e.g. `chaining_graph`
...
- `lib.rs`'s modules live in the eight role directories from
  `00-OVERVIEW.md` §"Target module layout"; the public crate path of every module
  is unchanged.
...
- **No claim-surface change.** This refactor touches file layout only; every
  ...
  data of unknown meaning; unsolved* (`00-OVERVIEW.md` §"Shared ground rules (apply
  to every brief)" — "Claim discipline is the crown jewel").
