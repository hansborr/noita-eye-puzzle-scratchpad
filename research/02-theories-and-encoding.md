...
- **Selection-effect caveat (not circularity).** **[Lymm]** Contiguity was not chosen in advance as the validation criterion — it emerged while researchers were testing reading orders and stood out as significant; the order is retained because of independently significant downstream structure (isomorphs, forbidden-successor patterns), not because contiguity alone was the selection test. The honest framing is a look-elsewhere correction: the probability figure measures "how unlikely is contiguity by chance given this reading-order family," which still carries a post-hoc selection effect worth quantifying (see `05-code-investigations.md` Experiment 1). Substitution-equivalent alternative reading orders change no computed statistic — every statistic here is substitution-invariant or conditioned on the fixed digit sequence. [likely a real signal, with a quantifiable but non-fatal selection effect]
...
**Assessment:** The 0–4 + "5=newline" coding is verbatim primary. The *specific* pixel-direction-per-digit (which orientation = "up", etc.) is shown only as an image in the primary sources, and Cipherbrain has warned "the order is different from the two pictures." **[Lymm]** The eye sprites are hardcoded in the engine's drawing function and can be extracted directly from the shipped binary, so the mapping is binary-verifiable, not merely image-sourced — maintainer-confirmed 2026-07-06, not yet independently re-extracted in this repo. A different digit↔direction labeling would only induce a fixed substitution on the ciphertext, cryptanalytically immaterial either way [likely — maintainer-attested, independent re-extraction pending; confirmed that five orientations + a newline marker exist].
...
### 3.2 ~83 internal states estimate — superseded
**Produced:** "No fewer than 20 internal states and statistically probably at least around 88, so it is likely 83 internal states" (Toboter) [disputed → superseded].
**Assessment:** Internally muddled (states "≥88" then concludes "83", i.e. *below* its own lower bound) and likely circular — 83 is exactly the number of distinct glyph values, so "#states = alphabet size" may be assumed rather than measured. Superseded, not merely disputed: under the surviving cipher-family theories (GAK on a near-S₈₃ state group), the true state space is S₈₃-scale (83! ≈ 10¹²⁴); "~83 states" is a historical artifact of the earlier, pre-GAK custom-Alberti-era framing, not a competing measurement [disputed → superseded].
...
### 3.6 Global transposition check
**Produced:** Under the accepted honeycomb order, the nine trigram streams have distinct lengths: `east1=99`, `west1=103`, `east2=118`, `west2=102`, `east3=137`, `west3=124`, `east4=119`, `west4=120`, `east5=114`. The documented shared runs nevertheless sit at the same ciphertext offsets across messages: all nine share `[66,5]` at offset 1, and the East/West counterpart runs include `East1/West1=24`, `East4/West4=20`, `East2/West2=2`, and `East3/West3=5` (see §6.3 and `src/nulls/perseus/mod.rs`) [confirmed as corpus-derived structure].
**Assessment:** This is evidence against a single global columnar/route/rail-fence transposition as the eyes' mechanism. Those transpositions are length-dependent permutations: messages of different lengths receive different index permutations, so a single shared route would not naturally preserve same-offset shared runs across all of these unequal streams. The point is deliberately limited: global transposition, or substitution followed by one shared global transposition route, is disfavored under the natural model, not mathematically impossible. It does not rule out per-message routes, adaptive/non-global schemes, or unrelated mechanisms that happen to preserve these anchors.
...
**Assessment:** A legitimate skeptical hypothesis that has now been tested in the workbench: the structure-matched generation-pipeline null does not manufacture the bounded 0..=82 contiguity, so this is not enough to explain the headline reading-layer anomaly. That still supports only "structured data," not plaintext [confirmed for the implemented null result; speculative as to meaning].
...
2. **No developer confirmation that the eyes encode recoverable plaintext.** The wiki's "known to contain information" is the wiki's own statistical inference, not a Nolla/Petri Purho quote. The widely-repeated "developers confirmed it's solvable" traces to an unsourced 2022 Hacker News intro line, with no Discord screenshot, quote, or dated statement behind it [confirmed: the meta-claim is unsourced] ([HN](https://news.ycombinator.com/item?id=33929442)). Documented dev statements exist for the *Cauldron Room* (Arvi, 2021), the *Orb-Room symbol* (Arvi/Antti/Olli denials), and the eyes themselves: in the same 2021 stream, Arvi confirmed the eye decorations *"do contain a message… do have a meaning"* and called them ~*"square root of minus 1"* hard (video youtube.com/watch?v=ItzQh6K3hP8; FuryForged Discord relay). Crucially that statement asserts *meaning*, not *solvability* — Arvi disclosed no method and implied extreme difficulty, so it does **not** supply the missing "developer confirmation that the eyes encode recoverable plaintext." [likely — relayed] ([Mysteries and Oddities](https://noita.wiki.gg/wiki/Mysteries_and_Oddities), [video](https://www.youtube.com/watch?v=ItzQh6K3hP8)).
...
**Bottom line:** The strongest, reproducible findings are structural (1036 trigrams, divisibility by 3, the [66,5] shared prefix, seed-determined locations, engine-hardcoded generation). The 0–82 anomaly is a likely signal of intentional structure but is order-dependent and carries a genuine look-elsewhere selection effect (see §1.2) — not the circular reasoning older framings implied. The leading cipher direction is a non-commutative, plaintext-driven permutation cipher (S₈₃/chaocipher-flavored, autokey/wheel), which is plausible but unproven; a single global transposition route is specifically disfavored by the unequal stream lengths plus same-offset anchors. Every concrete decode attempt — substitution, Vigenère, trifid, Polybius, atomic numbers, octahedron, PAM5, morse, binary — has failed or been debunked. The puzzle is genuinely unsolved.
