...
**Hidden assumption flagged:** "Eye Messages" is a community-coined label; no located Nolla/Petri Purho statement names the feature or calls it a puzzle. That the eyes *contain* a message is, however, dev-attested — Arvi's relayed-verbatim 2021-10-15 confirmation (see §3) — but that confirmation names no cipher, key, method, or plaintext, so it does not establish the feature as a solvable puzzle with recoverable content.
...
- **Cipher characterized as polyalphabetic, aperiodic, ~83 internal states, cipher/key constant across messages, with isomorphs.** [likely as a working hypothesis for polyalphabetic/aperiodic/isomorphs; the "~83 states" figure is superseded, not merely weak — see below] (https://noita.wiki.gg/wiki/Eye_Messages, https://docs.google.com/document/d/1QeagH8TklJsd8iribMtT5LIRL91laOUU_tFcVl7OOqA)
...
Superseding update: under the surviving cipher-family theories (GAK on a near-S₈₃ state group), the true state space is S₈₃-scale (83! ≈ 10¹²⁴) — the "~83 states" figure is a historical artifact of the earlier, pre-GAK custom-Alberti-era framing, not a competing measurement to reconcile with.
...
- **One** eye-specific dev statement exists. In the **same** 2021-10-15 Twitch stream the repo already cites for Arvi's Cauldron remark, Arvi also said (verbatim, relayed by FuryForged): *"for the eye decorations… I have confirmation that the eye decorations do contain a message… They actually do have a meaning"*, and rated their difficulty ~*"square root of minus 1… probably very difficult."* So a dev HAS confirmed the eyes carry an intentional message and are very hard — but disclosed **no** cipher, key, method, or solution, so the unsolved status and the solvability-to-plaintext caveat are unchanged. [likely — verbatim but relayed; primary anchor is the stream VOD] Still no eye-specific remarks from Purho / Harjola / Tiihonen, and the Orb-Room symbol was separately denied by Arvi, Antti, Olli. (https://discord.com/channels/453998283174576133/817530812454010910/899514286290898985, https://noita.wiki.gg/wiki/Mysteries_and_Oddities)
...
- **The specific direction-per-digit mapping (0=center, 1=up, 2=right, 3=down, 4=left).** **[Lymm]** The eye sprites are hardcoded in the function that draws the eyes and can be extracted directly from the shipped binary; the directions visible in-game match the base-5 digit values pulled from the raw hex. This is binary-verifiable, not merely image-sourced as earlier drafts of this doc treated it — maintainer-confirmed 2026-07-06, not yet independently re-extracted in this repo (an optional Ghidra follow-up in the `…-ghidra` worktree could close that gap). [likely — maintainer-attested; independent re-extraction pending] A different digit↔direction labeling would only induce a fixed substitution on the ciphertext — cryptanalytically immaterial, since every statistic in the workbench runs on the engine-fixed integer digit sequence (cross-validated byte-for-byte in Experiment 0), not on the cosmetic direction names. The repo owner, from direct in-game observation, separately confirms there are exactly 5 visually distinct orientations, corroborating the [confirmed] 0–4 inventory in §2.

---

## 4. Speculative — community inference without primary backing

- **Developer attitude / intended solvability.** That the eyes carry an intentional message is dev-attested, not speculative — Arvi confirmed it verbatim (relayed by FuryForged; see §3). What remains community inference is the stronger *design-intent* reading: that the eyes were authored as a deliberately **solvable** cipher with recoverable plaintext. Arvi disclosed no cipher, key, method, or solution and rated the difficulty "probably very difficult," so nothing carries the puzzle from "a message exists" to "the plaintext is recoverable." (Before the stream quote surfaced, even the bare intentionality was only *inferred* from the mod-gating, engine-hardcoding, statistical structure, and a Hempuli Roguelike Celebration talk that discussed Noita's secrets generally.) [bare intentional message: dev-attested (§3); authored solvability-to-plaintext: speculative] (https://www.youtube.com/watch?v=ItzQh6K3hP8)
...
- **"~83 internal states."** [disputed — superseded] Internally muddled on the wiki ("at least ~88 … so likely 83," i.e. fewer than its own lower bound) and risks circularity because 83 = the glyph-value count. Now superseded rather than merely disputed: the surviving cipher-family theories (GAK on a near-S₈₃ state group) imply an S₈₃-scale state space (83! ≈ 10¹²⁴); "~83 states" is a historical figure from the earlier custom-Alberti-era framing. (https://noita.wiki.gg/wiki/Eye_Messages)
...
1. **Reading-order selection effect (look-elsewhere, not circularity).** **[Lymm]** Contiguity was not a pre-registered validation criterion — it emerged while testing reading orders and stood out as significant; the order is *retained* because of downstream, independently significant structure (isomorphs, forbidden-successor), not because the community chose the order by a contiguity criterion. The honest statement: the contiguity p-value carries a post-hoc selection effect, properly quantified as a look-elsewhere correction (see Experiment 1 in `05-code-investigations.md`), not evidence of circular reasoning. The winning traversal is shown via images and could not be reproduced from the sources alone; naive orders fail (horizontal → 114 distinct values, 0–122; column-major → all 125). Substitution-equivalent alternative reading orders (the same 83 symbols on a non-contiguous range) change no computed statistic — every statistic in this workbench is substitution-invariant or conditioned on the fixed digit sequence. [confirmed that naive orders fail; look-elsewhere framing per Lymm 2026-07-06, not independently re-derived here] (computation; https://noita.wiki.gg/wiki/Eye_Messages)
...
3. **Circular state estimate — now superseded.** "~83 internal states" likely just restated the alphabet size (83 used values), not an independent measurement. [disputed → superseded] The surviving cipher-family theories (GAK on a near-S₈₃ state group) imply an S₈₃-scale state space (83! ≈ 10¹²⁴); "~83 states" reflects the historical custom-Alberti-era framing, not the current model.
...
- **Plausible but unproven [likely]:** the 0–82 reading-order anomaly; polyalphabetic/isomorph character; Ninji's Ghidra RE; seed-invariant content (now corroborated by direct in-game observation across multiple seeds — byte-for-byte cross-seed diff still pending); a dev-attested intentional message (Arvi's relayed-verbatim 2021-10-15 confirmation that the eyes "do contain a message," with no cipher/key/method/plaintext disclosed); the digit→direction mapping (maintainer-attested as binary-verifiable from the shipped binary, not yet independently re-extracted here — and cryptanalytically immaterial either way, see §3).
- **Not established [speculative]:** that the eyes were authored as a deliberately solvable cipher with recoverable plaintext (the bare intentional-message claim is dev-attested — see the [likely] tier above; its solvability is not); octahedron and permutation-cipher models; ship date/first discovery.
- **Should be retired or heavily qualified [disputed]:** "developers confirmed it's solvable" (traces to one unsourced HN line + AI-generated Grokipedia); "~83 internal states" (circular, and now superseded — surviving GAK/S₈₃ theories imply an S₈₃-scale state space); "no doubles / distance-4 spike" (false on raw order); "two major secrets" (self-contradictory, Cauldron now resolved); "Pyry is a dev."

The honest summary: the Eye Messages are a deliberately generated, engine-hardcoded, structurally regular artifact that is genuinely unsolved. That they carry an intentional message is dev-attested (Arvi, relayed-verbatim, 2021-10-15); but their status as a *solvable cipher with recoverable plaintext* is a well-motivated community belief — not a fact backed by any primary developer statement, as no dev has disclosed a cipher, key, method, or solution.
