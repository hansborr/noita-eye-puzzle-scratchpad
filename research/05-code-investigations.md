...
**Primary-observer update (2026-06-22):** the repo owner reports, from direct in-game observation across **multiple world seeds**, that the eye-message content is **identical** — corroborating seed-invariance from primary observation, which no prior source had. This is a **qualitative** confirmation (eyeballed equivalence), not the byte-for-byte trigram diff across ≥2 named seeds that remains the gold standard. Status moves from "unproven assumption" toward "directly corroborated"; the stronger form is still open and would become in-scope for the Rust workbench the moment a second-seed transcription is vendored under `research/data/` (the analysis half — a cross-seed diff test — needs no game access; only the second transcription does).
