...
has unknown-base known-plaintext recovery through exact state SAT and
independently bounded top-source retention. A 256-state cap closed all three
already-open beam misses but tied `16/16` with the 96-state default on a new
holdout, so the wider cap remains optional. Stop tuning this `n=7` fixture
family; any next community-GAK rung should test scaling or a broader generator
family, not point at the eyes.
...
1. `gak-unknown-base-recovery/` — unknown-base known-plaintext GAK/deck recovery
   on small planted instances is calibrated through an optional 256-state beam.
   The current `n=7` fixture family is saturated; only continue for Lymm's
   general-GAK request with a pre-registered scaling or broader-generator rung.
