...
| [handoff/gak-unknown-base-recovery/](handoff/gak-unknown-base-recovery/README.md) | **Built and scaling-calibrated community infrastructure for Lymm's stated interest.** Smaller hidden-base GAK/deck-cipher recovery with exact known-plaintext re-encryption. Base-completion marginalization recovers every retained plant. A fresh route-rank holdout improved `n=9` from `5/16` to `7/16` but tied `8/16` at `n=8`, so it remains opt-in; top-source ranking and the eyes' missing known plaintext remain the blockers. |
...
| [findings/eyes-structural-summary.md](findings/eyes-structural-summary.md) | **Publishable frontier summary.** One standalone synthesis of the eyes structural program: six-group transitivity pruning to `{A₈₃,S₈₃}` with `D₁₆₆` subsumed by the Full AGL sweep within-model [Lymm, verified]; AGL exclusion plus T02 robustness; perfect-isomorphism / Stutter sensitivity; G3 leak ceiling; Thread-4 fair honest-negative; and the standing conclusion that recovery remains blocked on missing key material, a method disclosure, or known plaintext — not on a fixed eye-symbol table (no such monoalphabetic table exists in a polyalphabetic cipher). |
...
| [data/practice-puzzles/SIX-RESULTS.md](data/practice-puzzles/SIX-RESULTS.md) | **Practice `six` exact candidate.** A rolling-cube/International-Morse instrument recovers `CUBE IS A GREAT TOY MODEL OF NON-COMMUTATIVITY.` with exact 139/139 replay on all three face relabelings and 0/1024 matched-null survivors. The missing pre-stream cube state leaves a disclosed exact `FUBE ...` first-mark twin, so external confirmation remains the formal close-out. |
