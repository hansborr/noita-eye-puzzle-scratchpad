...
| [findings/two-shadowfinish-substitution-candidate.md](findings/two-shadowfinish-substitution-candidate.md) | **Practice `two` breakthrough.** Fixed `shadowfinish` + `substfinish` yields a strong octal/Proto-Indo-European plaintext hypothesis under matched nulls. Still a candidate, not a verified decode, pending original-generator/ground-truth confirmation. |
