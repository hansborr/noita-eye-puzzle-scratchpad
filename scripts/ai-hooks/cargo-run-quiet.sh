...
# unchanged.
#
# Disable with NOITA_QUIET_OFF=1 or by creating .noita-quiet-off at the repo
# root. The hook is fail-open: malformed payloads, missing helper tools, parser
# uncertainty, or cache/fingerprint errors all emit continue so the original
...
ai_cargo_source_file() {
    case "$1" in
        *.rs | Cargo.toml | Cargo.lock | clippy.toml | deny.toml | \
            rustfmt.toml | rust-toolchain.toml | .cargo/*)
            return 0
            ;;
        *)
            return 1
...
ai_cargo_untracked_source_hashes() {
    local path hash

    while IFS= read -r -d '' path; do
        ai_cargo_source_file "$path" || continue
...
ai_cargo_worktree_fingerprint() {
    {
        ...
        ai_cargo_untracked_source_hashes | LC_ALL=C sort || exit 1
...
main() {
    ...
    timeout_seconds=$(ai_cargo_positive_integer_or_default "$AI_CARGO_QUIET_TIMEOUT" 1200)
