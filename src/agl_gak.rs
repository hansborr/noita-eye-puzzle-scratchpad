...
impl fmt::Display for AglGakError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Cipher(error) => write!(f, "cipher error: {error}"),
            ...
            Self::PositiveControlFailed { which } => {
                write!(f, "AGL-GAK positive control failed: {which}")
            }
            ...
            Self::ValueOutsideAlphabet { message_key, value } => write!(
                f,
                "message {message_key} value {value} is outside the 83-symbol reading alphabet"
            ),
