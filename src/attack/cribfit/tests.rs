...
#[test]
fn single_magnitude_mtf_is_crib_inconsistent_on_one() {
    // Single-magnitude MTF: the carrier value is 22/26 agreements (NOT the spec's
    // mistaken 0/26), but 22 < 26 means the windows do NOT decode identically, so it
    // is crib-INCONSISTENT and the filter must reject it.
    let geometry = documented_geometry();
    let candidate = mtf_candidate(&one_magnitudes(), Tokenization::Single, &geometry.anchors);
    assert!(
        !candidate.consistency.consistent,
        "single-magnitude MTF must be crib-inconsistent"
    );
    let len26 = candidate
        .consistency
        .anchors
