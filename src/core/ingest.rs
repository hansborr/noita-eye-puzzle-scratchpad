...
mod tests {
    ...
    fn rendered_drops_delimiter_and_whitespace() {
        let parsed = parse_sequence("012 345\n01", SequenceLayer::RenderedOrientation)
            .expect("rendered parse");
    ...
    fn rendered_rejects_non_digit_with_index() {
        let error = parse_sequence("01x", SequenceLayer::RenderedOrientation)
            .expect_err("non-digit must fail");
        match error {
    ...
    fn rendered_rejects_digit_above_five_with_index() {
        let error = parse_sequence("016", SequenceLayer::RenderedOrientation)
            .expect_err("digit > 5 must fail");
        match error {
    ...
    fn honeycomb_tolerates_extra_separators() {
        let parsed = parse_sequence("0,,12 , 82,", SequenceLayer::HoneycombReading)
            .expect("honeycomb parse");
        assert_eq!(parsed.glyphs, glyphs(&[0, 12, 82]));
    }
